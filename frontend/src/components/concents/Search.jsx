...
function SearchComponent() {
  ...
  const handle_save = async () => {
    try {
      const res = await axios.put(`http://localhost:3000/api/update/${editForm._id}`, editForm);
      toast(res.data.message, {
        style: {
          display: "flex",
          alignItems: "center",
            ...
            <div className=' card text-red-100 p-5 w-full max-w-2xl m-6 rounded-2xl bg-white/10 backdrop-blur-sm border border-white/20 shadow-2xl'>
                    ...
                    <div className="bg-white/10 backdrop-blur-sm rounded-lg p-3 border border-white/20">
                      {editMode ? (
                        <input
                          ...
                          className="bg-white/10 backdrop-blur-sm rounded-lg p-3 border border-white/20 text-white w-full"
                        />
                      ) : (
                    ...
                    <div className="bg-white/10 backdrop-blur-sm rounded-lg p-3 border border-white/20">
                      {editMode ? (
                        <input
                          ...
                          value={editForm.installment_date || ""}
                          onChange={handle_edit_change}
                          className="bg-white/10 backdrop-blur-sm rounded-lg p-3 border border-white/20 text-white w-full"
                        />
                      ) : (
                        <span className="text-white font-medium">{data.installmnt_date}</span>
                      )}
                    </div>
                  </div>
                </div>
                  ...
                  <div>
                    ...
                    <div className="bg-white/10 backdrop-blur-sm rounded-lg p-3 border border-white/20">
                      {editMode ? (
                        ...
                        <span className="text-white font-medium">{data.occupation}</span>
                      )}
                    </div>
                  </div>
                  <div>
                    ...
                    <div className="bg-white/10 backdrop-blur-sm rounded-lg p-3 border border-white/20">
                      {editMode ? (
                        ...
                        <span className="text-white font-medium">{data.phone_no}</span>
                      )}
                    </div>
                  </div>
                </div>
                  ...
                  <div>
                    ...
                    <div className="bg-white/10 backdrop-blur-sm rounded-lg p-3 border border-white/20">
                      {editMode ? (
                        <input
                          name="guranter"
                          value={editForm.guranter || ""}
                          onChange={handle_edit_change}
                          className="bg-white/10 backdrop-blur-sm rounded-lg p-3 border border-white/20 text-white w-full"
                        />
                      ) : (
                        <span className="text-white font-medium">{data.guranter}</span>
                      )}
                    </div>
                  </div>
                  <div>
                    ...
                    <div className="bg-white/10 backdrop-blur-sm rounded-lg p-3 border border-white/20">
                      {editMode ? (
                        <input
                          name="guranter_phone_no"
                          value={editForm.guranter_phone_no || ""}
                          onChange={handle_edit_change}
                          className="bg-white/10 backdrop-blur-sm rounded-lg p-3 border border-white/20 text-white w-full"
                        />
                      ) : (
                        <span className="text-white font-medium">{data.guranter_phone_no}</span>
                      )}
                    </div>
                  </div>
                </div>
              ...
              {editMode ? (
                <div className='flex justify-end gap-4 mt-8 pt-4 border-t border-white/20'>
                  <button onClick={handle_save} className="flex items-center gap-2 px-4 py-2 cursor-pointer bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 rounded-lg font-medium hover:scale-105 transition-all duration-300 shadow-lg">

                    <span>Save</span>
                  </button>
                  <button onClick={() => setEditMode(false)} className="flex items-center gap-2 px-4 py-2 cursor-pointer bg-gradient-to-r from-red-600 to-red-700 hover:from-red-700 hover:to-red-800 rounded-lg font-medium hover:scale-105 transition-all duration-300 shadow-lg">
